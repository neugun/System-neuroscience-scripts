...
% DataType=0;
% timerange=[0;9000]; 
% Timerange=[0;9000] 则Analysi period from 0s to 9000s in the file. 
% Timerange=[300 700 1400;500 1000 4000] 则Analysi period from 300s to 500s, 700s to 1000s and 1400 to 4000s.
% range=[-4 4]; range是考虑event前后几秒的数据
% findname=‘E:\Hu Group Data\NaviReward-M27-100914002-f.nex’
% DataParam='spkc14'; %Nex文件中神经元的名字
...
%  timerange=[1;2000];
%  [output DataType DataTime]=GetDataNex(fileName,DataParam,timerange);
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
output=[];
if isstruct(DataParam)                                                  % isstruct(S) returns logical true (1) if S is a structure and logical false (0) otherwise.
    
 [nexFile] = readNexAll(fileName,DataParam.Name);                          % [nexFile]= readNexFile(dirname);                                                      %    
else
    
 [nexFile] = readNexAll(fileName,DataParam);                              % [nexFile]= readNexFile(dirname);                                                       
...
switch DataType                                                   
    ...
    case adfreq                                                  %%%%%%%%%%%%%%adfreq表示采样频率         
        ...
        tempData=[];
        timerangeI=round(timerange.*adfreq)+1;              %取证。采样频率。
